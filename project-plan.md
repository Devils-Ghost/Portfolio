...
**This file stays a clean, forward-looking plan.** Checkboxes reflect current status; resolved decisions are stated in their settled form, not their revision history. Anything discovered, corrected, or superseded _while a phase was being executed_ — corrections found mid-build, review-pass notes, "we changed our mind because X" — lives in the companion file `plan-progress.md` instead, tagged by phase. Read that file when you want the story of how a phase actually went; read this one when you want to know what's next.
