...
**Three corrections found while executing this phase:**
...
3. **§1.4 #8's Inter claim was wrong.** See the corrected row.

**Accent tokens were done differently to §D6.** The ~100 `blue-*` references aren't hex literals — they're already Tailwind tokens. Redefining `--color-blue-{300,400,500,600,900}` in `@theme` (seeded with v4's exact defaults) makes them ours with zero call-site churn, and retuning the accent is now a five-line edit. `--color-accent` / `--color-accent-soft` exist as semantic aliases for new code.
