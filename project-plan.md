...
| 8   | **`globals.css` still has Next.js starter cruft**: `--font-geist-sans`/`--font-geist-mono` referenced but never defined; a light-mode `:root` and `prefers-color-scheme` block that are dead (html is hardcoded `dark`, body is `bg-black`); and `body { font-family: Arial, Helvetica, sans-serif }` **overriding the Inter you load via `next/font`** | You are not actually rendering in Inter. The splash screen even hardcodes Arial to match. Worth knowing before you "keep the same theme."                                                                                                                                                                                                       |
...
- [ ] Fix the four placeholder URLs (§1.4 #6) and redeploy — **do this first, it's 2 minutes and it's live right now**
- [ ] Clean `globals.css`: drop the dead light-mode block and the phantom Geist vars; remove the `font-family: Arial` override so Inter actually renders
- [ ] Extract design tokens into `@theme` (§D6); replace the hex literals
- [ ] `Modal`: Escape-to-close, focus trap, focus restore, `role="dialog"` + `aria-modal`, ref-counted scroll lock
- [ ] `sessionStorage` skip for the splash screen on repeat visits
- [ ] Directory restructure per §4 (folders only, no logic changes)
- [ ] `tsconfig` strict mode on; ESLint incl. `react-hooks/rules-of-hooks` as an **error**; Prettier
- [ ] `git init` if needed, connect to GitHub, Vercel preview deploys on PR

**Exit:** site looks pixel-identical, `npm run build` is clean, lint is clean, no `any` in component props.
